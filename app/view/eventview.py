...
class EventView:
    ...
    def eventview(self, user, event, userDAO):
        ...
        if user.id == event.id_user:
            ...
            tabledonneessolo.add_row("Gestionnaire",
                                     f"{userDAO.get_user(event.id_user).nom}")
        tabledonneessolo.add_row("Date de début",
                                 f"{event.event_date_start.strftime(
                                     '%Y-%m-%d %H:%M:%S')}")
        tabledonneessolo.add_row("Date de fin",
                                 f"{event.event_date_end.strftime(
                                     '%Y-%m-%d %H:%M:%S')}")
        ...
        if event.id_user == user.id and user.authorisation('Support'):
            tablechoix.add_row("MS <Nouvelle Donnée>",
                               "Modifier la date de début <Nouvelle "
                               "Donnée au format : JJ/MM/AAAA HH:MM>")
            tablechoix.add_row("ME <Nouvelle Donnée>",
                               "Modifier la date de fin <Nouvelle Donnée "
                               "au format : JJ/MM/AAAA HH:MM>")
            tablechoix.add_row("ML <Nouvelle Donnée>",
                               "Modifier la localisation par"
                               " <Nouvelle Donnée>")
            tablechoix.add_row("MA <Nouvelle Donnée>",
                               "Modifier le nombre de participants"
                               " par <Nouvelle Donnée>")
            tablechoix.add_row("MN <Nouvelle Donnée>",
                               "Modifier les notes par <Nouvelle Donnée>")
            tablechoix.add_row("SUPPRIMER", "[blue]Supprime "
                               "définitivement[/blue] l'évènement ")
        else:
            tablechoix.add_row("[red][strike]MS <Nouvelle Donnée>[/red]"
                               "[/strike]", "[red]Modifier la date de début "
                               "<Nouvelle Donnée au format : JJ/MM/AAAA HH:MM>"
                               "[/red]🔒")
            tablechoix.add_row("[red][strike]ME <Nouvelle Donnée>[/red]"
                               "[/strike]", "[red]Modifier la date de fin"
                               " <Nouvelle Donnée au format : JJ/MM/AAAA "
                               "HH:MM>[/red]🔒")
            tablechoix.add_row("[red][strike]ML <Nouvelle Donnée>[/red]"
                               "[/strike]", "[red]Modifier la localisation "
                               "par <Nouvelle Donnée>[/red]🔒")
            tablechoix.add_row("[red][strike]MA <Nouvelle Donnée>[/red]"
                               "[/strike]", "[red]Modifier le nombre de"
                               " participants par <Nouvelle Donnée>[/red]🔒")
            tablechoix.add_row("[red][strike]MN <Nouvelle Donnée>[/red]"
                               "[/strike]", "[red]Modifier les notes par "
                               "<Nouvelle Donnée>[/red]🔒")
            tablechoix.add_row("[red][strike]SUPPRIMER[/red][/strike]",
                               "[red]Supprime définitivement "
                               "l'évènement [/red]🔒")
        ...
        console.print("")
        if event.id_user == user.id and user.authorisation('Support'):
            valid_choices = ["SUPPRIMER", "RET", "QUIT"]
        ...
        while True:
            console.print("Votre choix [#AAAAAA]( MA 563, MN Mes notes, "
                          "MS 25/12/2025 15:50 ...)[/#AAAAAA] :")
            choix = input('==>')
            if choix in valid_choices:
                ...
                if choix.startswith("MS ") or choix.startswith("ME "):
                    ...
                    if not pattern.match(date_str):
                        print("Erreur : Format de la date non valide. "
                              "Format accepté : JJ/MM/AAAA HH:MM")
                    try:
    ...
    def myMensualEvents(self, user, events):
        ...
        if user.authorisation('Support'):
            ...
            def create_table(liste, car):
                if car == "MY":
                    table = Table(title="Evènements du mois "
                                  f"{calendar.month_name[current_month]}",
                                  box=None)
                else:
                    table = Table(title=f"Evènements du mois "
                ...
                for event in liste:
                    if (event["id_user"] if isinstance(event, dict) else event.id_user) == user.id:
                        ...
                        table.add_row(str(event_id), company_name,
                                      event_date_start.strftime(
                                          "%Y-%m-%d %H:%M:%S"),
                                      str(attendees))
                        if car == "MY":
            ...
            console.print("[bold underline]Calendrier  "
                          f"{calendar.month_name[current_month]} :[/]")
    ...
    def myTotalEvents(self, user, events):
        ...
        if user.authorisation('Support'):
            ...
            def create_table(liste):
                table = Table(title="Evènements", box=None)
                table.add_column("id", justify="left", style="green",
                                 no_wrap=True)
                table.add_column("Entreprise", justify="left", style="white",
                                 no_wrap=True)
        ...
        def create_table(liste):
            ...
            for event in liste:
                ...
                if (event["id_user"] if isinstance(event, dict) else event.id_user) == user.id:
                    ...
                    table.add_row(f"{str(event_id)}🔒", company_name,
                                  event_date_start.strftime(
                                      "%Y-%m-%d %H:%M:%S"))
            return table
